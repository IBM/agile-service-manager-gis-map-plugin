...
export default function createClusterGroup(map, config) {
    ...
    clusterGroup.on('click', function (marker) {
        if ( marker.sourceTarget.feature.properties.aggreationZoomLevel) {
            const zoomLevel = marker.sourceTarget.feature.properties.aggreationZoomLevel;
            if (zoomLevel >= 0 && zoomLevel <=15) {
                map.setView(marker.latlng, zoomLevel + 4);
            }
            console.log('Zoom to aggreationZoomLevel', marker, marker.sourceTarget.feature.properties.aggreationZoomLevel);
